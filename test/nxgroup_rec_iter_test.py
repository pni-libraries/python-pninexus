...
class nxgroup_test(unittest.TestCase):
    ...
    def setUp(self):
        self.gf = create_file("test/nxgroup_rec_iter_test.nxs",overwrite=True)
        self.root = self.gf.root()
        entry = self.root.create_group("entry",nxclass="NXentry")
        entry.create_group("instrument",nxclass="NXinstrument")
        entry.create_group("control",nxclass="NXmonitor")
