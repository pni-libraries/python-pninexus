...
template<typename FIELDT> class nxfield_wrapper
{
    ...
    public:
        ...
        boost::python::object read() const
        {
            ...
            {
                std::vector<npy_intp> index(_field.rank());
                std::fill(index.begin(),index.end(),0);
               
                PyArrayObject *arr_ptr = reinterpret_cast<PyArrayObject*>(np_array.ptr());
                PyObject* ptr = PyArray_GETITEM(arr_ptr,
                                                PyArray_GetPtr(arr_ptr,index.data()));
                Py_INCREF(ptr);
                handle<> h(ptr);
                np_array = object(h);
            }
