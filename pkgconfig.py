...
class package(object):
    ...
    def _get_library_dirs(self):
        result = self._decode(execute([self.command,'--libs-only-L',self.name]))
        result = split_result(result,'-L')
        return strip_string_list(result)

    def _get_include_dirs(self):
        result = self._decode(execute([self.command,'--cflags-only-I',self.name]))
    ...
    def _get_compiler_flags(self):
        ...
        total_result = self._decode(execute([self.command,'--cflags',self.name]))
        ...
        includes = self._decode(execute([self.command,'--cflags-only-I',self.name]))
